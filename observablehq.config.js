...
export default {
  ...
  dynamicPaths: []
    ...
    .concat(quizes.filter(d => d.subject == "math").flatMap(d => d.codes).map(code => `/data/raw-${code}.md`))
    ...
    .concat(quizes.filter(d => d.subject == "math").flatMap(d => d.codes).filter(code => "M9D-2025").map(code => `/notebook-${code}`)))    
