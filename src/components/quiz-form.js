...
export function renderQuizWithInputs(params) {
...
function chunkMetadataByInputs(metadata, subject, selectedIds = []) {
    const leafs = Object.groupBy(getAllLeafsWithAncestors(convertTree(metadata), (parent, child) => {
...
function quizQuestions({ questions, quizQuestionsMap, subject, displayOptions }) {
    const { useBreakInside, useBreakBefore, useAIHelpers, useFormControl } = displayOptions;
    const inputsStore = {};
    return [questions.map(([code, ...id]) => {
            ...
            const optionsMap = Object.fromEntries(quizBuilder.questions.map((d) => [d.id, d.options]));
            ...
            return html `<div class=${useBreakInside ? 'avoid' : ''}>${chunks.flatMap(([inline, g], i) => {
                ...
                if (inline) {
                    ...
                    const metadataMap = Object.fromEntries(leafs.map((data) => {
                        const d = data.leaf.data.node;
                        const label = data.leaf.data.id;
                        const id = parseQuestionId(label, subject);
                        return [id, d];
                    }));
                    const context = Object.fromEntries(leafs.map((data) => {
                        const d = data.leaf.data.node;
                        const label = data.leaf.data.id;
                        const id = parseQuestionId(label, subject);
                        ...
                        return [
                            id,
                            toTooltipInput(inputBy.kind == "options"
                                ? Inputs.button(optionsMap[id]?.map((opt) => [opt.name, (value) => opt]))
                    ...
                    return html `<div class="avoid">${codeComponent(0)}${useFormControl
                        ...
                        : mdPlus.unsafe(quizBuilder.content(ids, { render: 'content' }))}</div>`;
                }
                else {
...
function parseQuestionId(id, subject) {
    ...
    return parseInt((subject === "cz" || subject === "math") ? parts[0] : parts[1], 10);
}
function toTooltipInput(input) {
    const s = useInput(input);
    const tooltip = html `<span>
  ${toolTipper(html `<button class="btn btn--primary"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-hand-index" viewBox="0 0 16 16">
...
</svg></button>`, () => html `${input}`, {
        onShow: (instance) => {
            instance.popper.querySelector("input")?.focus();
            instance.popper.querySelectorAll("button").forEach((el) => el.addEventListener("click", (event) => {
                instance.hide();
            }));
        }
    })}</span>`;
...
function toTemplate(template, context, formatValue = (key) => value => value) {
    ...
    const dynamicEvaluator = (templateStr, context) => {
        const func = new Function("rhtml", "context", "computed", "formatValue", `return rhtml\`${templateStr}\`;`);
...
function isEmptyAnswer(value) {
    ...
    if (values.length > 0 && values.every((d) => isEmptyOrWhiteSpace(d))) {
        return true;
    }
    return false;
}
function toInputGroup({ type, value, label }, inputBy) {
...
function hasValueProperty(n) {
    if (Object.hasOwn(n, "value"))
        return true;
    return (typeof n === "object" && Object.hasOwn(n.constructor.prototype, "value"));
}
...
function toForm(fn, options = {}) {
    const { submitLabel = 'Submit', resetLabel = 'Reset', required = false, resubmit = true, width = 'fit-content', justify = 'start', label, } = options;
    const onSubmit = () => {
        value = input.value;
    ...
    const template = inputs => html `<div>${label ? html `<span class="form-group-label">${label}</span>` : ''}<div>${Array.isArray(inputs) ? inputs : Object.values(inputs)}${footer}</div>`;
...
function sortableInput(array, options = {}) {
    let { listStyle = "padding-left:0", itemStyle = `
       ...
       border-radius: 10px;
       background: #EEE;`, format = d => d, footer } = options;
