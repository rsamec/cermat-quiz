...
import { cont, ratio, ctor, comp, compRelative, compRatio, ctorComplement, ctorLinearEquation, sum } from "../../components/math.js";
...
export function exampleCompareTotalMultiple({ input }: {
  input: {
    second: boolean
    total: number;
    partRatio: number;
  ...
  const template1 = highlight => `
    Všechny výrobky dohromady stojí ${input.total} Kč.
    Výrobek A je relativně o ${`${Math.abs(input.partRatio)} ${input.partRatio > 0 ? 'dražší' : 'levnější'}`} než výrobek B.
    Výrobek ${input.second ? "A" : "B"} je relativně o ${`${Math.abs(input.partRatio2)} ${input.partRatio2 > 0 ? 'dražší' : 'levnější'}`} než výrobek C.
    Kolik stojí výrobek C?`;
  ...
  const first = {
    comp1: axiomInput(compRelative(agentFirst, agentSecond, input.partRatio), 2),
    comp2: axiomInput(compRelative(input.second ? agentFirst : agentSecond, agentThird, input.partRatio2), 3),
    input: axiomInput(cont(input.second ? agentSecond : agentFirst, "x", entity), 1),
    template: template1
  }
  const second = {
    comp1: axiomInput(compRelative(agentFirst, agentSecond, input.partRatio), 2),
  ...
  const testCases = (first) => {
    const tempVariable = deduce(first.comp1, first.input);
    const result = deduce(
          deduce(
            first.input,
            tempVariable,
            deduce(last(tempVariable), first.comp2),
            sum(agentFirst)
          ),
          axiomInput(cont("celkem", input.total, entity), 4),
          ctorLinearEquation(input.second ? agentSecond : agentFirst, { entity }, "x")
        )
    const temp = deduce(last(result), first.comp1)        
    return [
      {
        deductionTree: deduce(
          result,
          temp,
          deduce(last(temp), first.comp2),
          ctor("tuple")          
        ) , template: first.template
      }]
  }
