...
function quotaRuleEx(a, quota3) {
  ...
  return {
    ...
    entity: a.entity,
    quantity: a.agent === quota3.agentQuota ? a.quantity * quota3.quantity : a.quantity / quota3.quantity
  };
}
...
function toDeltaEx(a, b, last4) {
  ...
  return { kind: "delta", agent: { name: last4.agent?.name ?? b.agent, nameBefore: a.agent, nameAfter: b.agent }, quantity: b.quantity - a.quantity, entity: a.entity, unit: a.unit };
}
...
function inferenceRuleEx(...args) {
  ...
  if (a.kind === "rate" && b.kind === "rate" && last4.kind === "ratios") {
    ...
    const kind = last4?.kind;
    return kind === "scale" ? mapRatiosByFactor(b, a.quantity) : kind === "invert-scale" ? mapRatiosByFactor(b, 1 / a.quantity) : kind === "nth-factor" ? nthPartFactorBy(b, a, last4) : kind === "nth-part" ? partToPartRule(a, b, last4) : partToPartRule(a, b);
  } else if (a.kind === "ratios" && b.kind == "cont") {
    const kind = last4?.kind;
    return kind === "scale" ? mapRatiosByFactor(a, b.quantity) : kind === "invert-scale" ? mapRatiosByFactor(a, 1 / b.quantity) : kind === "nth-factor" ? nthPartFactorBy(a, b, last4) : kind === "nth-part" ? partToPartRule(b, a, last4) : partToPartRule(b, a);
...
function convertToPartToPartRatios2(b, a) {
  ...
  return {
    ...
    result,
    options: [
      { tex: `(${formatRatio2(Math.abs(b.ratio))}) ku 1`, result: result.ratios.map((d) => formatRatio2(d)).join(":"), ok: true },
      { tex: `(1 / ${formatRatio2(Math.abs(b.ratio))}) ku 1`, result: result.ratios.map((d) => formatRatio2(d)).join(":"), ok: false }
    ]
  };
}
...
function transferRule2(a, b, transferOrder) {
  ...
  return {
    ...
    result,
    options: [
      { tex: `${formatNumber2(a.quantity)} ${transferOrder === "before" && a.agent == b.agentSender.name ? " + " : " - "} ${formatNumber2(Math.abs(b.quantity))}`, result: formatNumber2(result.quantity), ok: a.agent == b.agentSender.name },
      { tex: `${formatNumber2(a.quantity)} ${transferOrder !== "before" && a.agent == b.agentSender.name ? " - " : " + "} ${formatNumber2(Math.abs(b.quantity))}`, result: formatNumber2(result.quantity), ok: a.agent == b.agentReceiver.name }
    ]
  };
}
...
function partToWholeRuleEx2(a, b) {
  ...
  return matchAgent2(b.whole, a) ? { kind: "cont", agent: b.part, entity: a.entity, quantity: a.quantity * b.ratio, unit: a.unit } : { kind: "cont", agent: b.whole, entity: a.entity, quantity: a.quantity / b.ratio, unit: a.unit };
}
...
function toDeltaEx2(a, b, last22) {
  ...
  return { kind: "delta", agent: { name: last22.agent?.name ?? b.agent, nameBefore: a.agent, nameAfter: b.agent }, quantity: b.quantity - a.quantity, entity: a.entity, unit: a.unit };
}
...
function inferenceRuleEx2(...args) {
  ...
  if (a.kind === "rate" && b.kind === "rate" && last22.kind === "ratios") {
    ...
    const kind = last22?.kind;
    return kind === "scale" ? mapRatiosByFactor2(b, a.quantity) : kind === "invert-scale" ? mapRatiosByFactor2(b, 1 / a.quantity) : kind === "nth-factor" ? nthPartFactorBy2(b, a, last22) : kind === "nth-part" ? partToPartRule2(a, b, last22) : partToPartRule2(a, b);
  } else if (a.kind === "ratios" && b.kind == "cont") {
    const kind = last22?.kind;
    return kind === "scale" ? mapRatiosByFactor2(a, b.quantity) : kind === "invert-scale" ? mapRatiosByFactor2(a, 1 / b.quantity) : kind === "nth-factor" ? nthPartFactorBy2(a, b, last22) : kind === "nth-part" ? partToPartRule2(b, a, last22) : partToPartRule2(b, a);
  } else if (a.kind === "cont" && b.kind === "comp-diff") {
...
function krouzky() {
  ...
  const florbalDiff = deduce2(
    ...
    deduce2(
      ...
      tanecni,
      lezeckaStena,
      sum(`zadan\xE9 \xFAdaje`, [], entityPercent2, entityPercent2)
    ),
    ctor("comp-diff")
  );
    ...
    pocetMinut: {
      deductionTree: deduce(
        ...
        deduce(
          ...
          sum("konec promitani", [], { entity: entity3, unit }, { entity: entity3, unit })
        ),
  ...
  return {
    ...
    zaku: {
      deductionTree: deduce(
        ...
        sum("celkem", [entityDvojic, entityTrojic], entity3, entity3)
      )
    }
  };
}
    ...
    deductionTree: deduce(
      deduce(
        ...
        deduce(
          ...
          hvozdiky,
          sum("dohromady", [kopretinyLabel, hvozdikyLabel], entity3, entity3)
        ),
  ...
  return {
    deductionTree: deduce(
      ...
      deduce(
        last(deti),
        dospely,
        sum("celkem", [detiLabel, dospeliLabel], entity3, entity3)
      ),
      ctorPercent()
    )
  };
  ...
  return {
    ...
    cisla3: {
      deductionTree: to(
        ...
        cont("prvn\xED nezn\xE1m\xE9 \u010D\xEDslo", last(soucet).quantity + last(rozdil).quantity, entity3),
        commonSense("druh\xE9 nezn\xE1m\xE9 \u010D\xEDslo sn\xED\u017E\xEDme o polovinu rozd\xEDlu"),
        cont("druh\xE9 nezn\xE1m\xE9 \u010D\xEDslo", last(soucet).quantity - last(rozdil).quantity, entity3)
      )
    }
  };
}
    ...
    cerneKoralky: {
      deductionTree: deduce(
        ...
        to(
          ...
          cont("b\xEDl\xE9", last(colorQuota).quantity + 1, entity3)
        ),
...
function ctverce() {
  ...
  const strana2 = deduce(
    deduce(
      strana1,
      strana1Add,
      sum(tempMatchSiteLabel2, [], { entity: entity3, unit }, { entity: entity3, unit })
    ),
  ...
  const width3 = deduce(
    last(strana2),
    widthAdd,
    sum(tempMatchSiteLabel3, [], { entity: entity3, unit }, { entity: entity3, unit })
  );
  return {
    obvodObrazec2: {
      deductionTree: deduce(
    ...
    obrazecWidthToLength: {
      deductionTree: deduce(
        ...
        deduce(
          ...
          sum("del\u0161\xED strana 3. obrazce", [], { entity: entity3, unit }, { entity: entity3, unit })
        )
      )
    }
  };
...
function build7({ input }) {
  ...
  const lastMeritko = last2(meritko);
  const dTree2 = deduce2(
  ...
  return [
    { deductionTree: meritko, template: (highlight2) => highlight2`${() => templateBase(highlight2)}${template1}` },
...
function example2({ input }) {
  ...
  const deductionTree = deduce2(
    vynos,
    deduce2(danPercent, last2(vynos))
  );
  return { deductionTree, template };
}
  ...
  return {
    ...
    vzestupVyska: {
      deductionTree: deduce(
        to(
          vzestupHladiny,
          cont("vzestup hladiny", last(vzestupHladiny).quantity, entity3, unit)
        ),
        ctorUnit("mm")
      )
    }
      ...
      deductionTree: deduce(
        deduce(
          ...
          deduce(
            ...
            sum("sou\u010Det dvou \xFAhl\u016F", [], { entity: entity3, unit }, { entity: entity3, unit })
          ),
...
function zahon2() {
  ...
  const unit1d = "cm";
  const entity3 = "po\u010Det";
  const pocetRostlin = cont("rostliny", 65, entity3);
      ...
      deductionTree: deduce(
        deduce(
          deduce(
            to(
              pocetRostlin,
              commonSense(`rozklad na prvo\u010D\xEDsla:${primeFactorization([pocetRostlin.quantity]).join(",")}`),
...
function bazen() {
  ...
  return {
    deductionTree: deduce(
      ...
      sum("baz\xE9n celkem", [], { entity: entity3d, unit: unit3d }, { entity: entity3d, unit: unit3d })
    )
  };
}
function pelhrimov() {
    ...
    deductionTree: deduce(
      deduce(
        ...
        deduce(
          ...
          last(praveRameho),
          sum("dvojice sou\u010Det", ["troj\xFAheln\xEDk bod A", "troh\xFAheln\xEDk bod B"], { entity: entity3, unit }, { entity: entity3, unit })
        ),
...
function pravouhlyLichobeznik() {
  ...
  const obvod = deduce(
    ...
    spodniZakladna,
    height,
    sum(`${agentLabel} obvod`, [], { entity: entity3, unit }, { entity: entity3, unit })
  );
  return {
    obsah: {
      deductionTree: deduce(
    ...
    obvodRovnobeznik: {
      deductionTree: deduce(
        ...
        prepona,
        sum("rovnob\u011B\u017En\xEDk obvod", [], { entity: entity3, unit }, { entity: entity3, unit })
      )
    }
  };
}
