...
function toTooltipInput(input) { 
  ...
  const tooltip = rhtml`<span>
  ${toolTipper(    
    rhtml`<div class="btn"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-hand-index" viewBox="0 0 16 16">
...
function toTemplate(
  ...
  const interpolateString = (str) => {
    ...
    return str.replace(maskPlaceholderPattern, (match, maskNumber) => {
      return `(${maskNumber}) **(formatValue(context[${maskNumber}][1]))}**  \${context[${maskNumber}][0]}`;
    });
  };
