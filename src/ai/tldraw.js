...
function isOperationPredicate(value) {
  return ["sum", "sum-combine", "product", "product-combine", "gcd", "lcd"].includes(value.kind) || ["scale", "scale-invert", "slide", "slide-invert", "diff", "complement", "unit", "round", "floor", "ceil", "invert"].includes(value.kind);
}
...
function inferInvertToRule(a, b) {
  const result = {
    ...a,
    quantity: isNumber2(a.quantity) ? a.quantity * -1 : wrapToQuantity(`a.quantity * -1`, { a })
  };
  return {
    name: "invertRule",
    inputParameters: extractKinds(a, b),
    question: isNumber2(a.quantity) ? `P\u0159eve\u010F ${formatNumber(a.quantity)} ${formatEntity(a)} na opa\u010Dnou hodnotu.` : `P\u0159eve\u010F na opa\u010Dnou hodnotu.`,
    result,
    options: isNumber2(a.quantity) && isNumber2(result.quantity) ? [] : []
  };
}
...
function rateRule(a, rate) {
  ...
  return {
    ...
    quantity: isEntityBase2 ? isNumber2(a.quantity) && isNumber2(rate.quantity) && isNumber2(rate.baseQuantity) ? a.quantity / (!isUnitRate ? rate.quantity / rate.baseQuantity : rate.quantity) : !isUnitRate ? wrapToQuantity(`a.quantity / (rate.quantity/rate.baseQuantity)`, { a, rate }) : wrapToQuantity(`a.quantity / rate.quantity`, { a, rate }) : isNumber2(a.quantity) && isNumber2(rate.quantity) && isNumber2(rate.baseQuantity) ? a.quantity * (!isUnitRate ? rate.quantity / rate.baseQuantity : rate.quantity) : !isUnitRate ? wrapToQuantity(`a.quantity * (rate.quantity/rate.baseQuantity)`, { a, rate }) : wrapToQuantity(`a.quantity * rate.quantity`, { a, rate })
  };
}
...
function toQuotaRule(a, quota, last) {
  const isQuota = isSameEntity(a, quota);
  const quantity = isNumber2(a.quantity) && isNumber2(quota.quantity) ? Math.floor(a.quantity / quota.quantity) : wrapToQuantity(`floor(a.quantity / quota.quantity)`, { a, quota });
  return {
    kind: "quota",
...
function inferToQuotaRule(a, quota, last) {
...
function rateQuotaRule(a, rate, moduloType = "floor") {
  ...
  if (moduloType === "ceil") {
    quantities = {
      quantity: isNumber2(a.quantity) && isNumber2(rate.quantity) && isNumber2(rate.baseQuantity) ? isUnitRate ? Math.ceil(a.quantity / rate.quantity) : Math.ceil(a.quantity / rate.quantity) * rate.baseQuantity : isUnitRate ? wrapToQuantity(`ceil(a.quantity / rate.quantity)`, { a, rate }) : wrapToQuantity(`ceil(a.quantity / rate.quantity) * rate.baseQuantity`, { a, rate }),
      restQuantity: isNumber2(a.quantity) && isNumber2(rate.quantity) ? a.quantity - rate.quantity * Math.ceil(a.quantity / rate.quantity) : wrapToQuantity(`a.quantity - rate.quantity * ceil(a.quantity / rate.quantity)`, { a, rate })
    };
  } else if (moduloType === "floor") {
    quantities = {
      quantity: isNumber2(a.quantity) && isNumber2(rate.quantity) && isNumber2(rate.baseQuantity) ? isUnitRate ? Math.floor(a.quantity / rate.quantity) : Math.floor(a.quantity / rate.quantity) * rate.baseQuantity : isUnitRate ? wrapToQuantity(`floor(a.quantity / rate.quantity)`, { a, rate }) : wrapToQuantity(`floor(a.quantity / rate.quantity) * rate.baseQuantity`, { a, rate }),
      restQuantity: isNumber2(a.quantity) && isNumber2(rate.quantity) ? a.quantity - rate.quantity * Math.floor(a.quantity / rate.quantity) : wrapToQuantity(`a.quantity - rate.quantity * floor(a.quantity / rate.quantity)`, { a, rate })
    };
  } else {
...
function inferRateQuotaRule(a, rate, last) {
  ...
  return {
    name: rateRule.name,
    inputParameters: extractKinds(a, rate),
    question: isNumber2(a.quantity) && isNumber2(rate.quantity) && isNumber2(result.quantity) ? `Rozd\u011Bl ${formatNumber(a.quantity)} ${formatEntity({ entity: a.entity, unit: a.unit })} postupn\u011B na skupiny velikosti ${formatNumber(rate.quantity)} ${formatEntity(rate.entity)}` : "",
    result,
    options: isNumber2(a.quantity) && isNumber2(rate.quantity) && isNumber2(result.quantity) && isNumber2(rate.baseQuantity) ? [
...
function inferenceRuleEx(...args) {
  ...
  if (["sum-combine", "sum", "product-combine", "product", "gcd", "lcd", "sequence", "tuple", "eval-expr", "eval-formula", "alligation"].includes(last?.kind)) {
    ...
    if (kind === "quota")
      return inferToQuotaRule(a, b, last);
    if (kind === "delta")
    ...
    return kind === "ratio" ? inferToPartWholeRatio(b, a, last) : kind === "quota" && a.kind === "cont" ? inferRateQuotaRule(a, b, last) : inferRateRule(a, b);
  } else if (a.kind === "rate" && (b.kind == "cont" || b.kind === "quota" || b.kind === "rate")) {
    return kind === "ratio" ? inferToPartWholeRatio(a, b, last) : kind === "quota" && b.kind === "cont" ? inferRateQuotaRule(b, a, last) : inferRateRule(b, a);
