...
function toComparisonAsRatioEx(a, b) {
  ...
  return {
    ...
    agentB: b.part,
    agentA: a.part,
    ratio: isNumber2(a.ratio) && isNumber2(b.ratio) ? 1 + (a.ratio - b.ratio) : wrapToRatio(`1 + (a.ratio - b.ratio)`, { a, b })
  };
}
...
function roundTo(a, b) {
  const result = {
    ...a,
    quantity: isNumber2(a.quantity) ? Math.round(a.quantity) : wrapToQuantity(`round ${a.quantity}`, { a })
  };
  return {
    question: isNumber2(a.quantity) ? `Zaokrouhli ${formatNumber(a.quantity)} ${formatEntity(a)} na cel\xE9 \u010D\xEDslo.` : `Zaokrouhli na cel\xE9 \u010D\xEDslo.`,
    result,
...
function ratiosConvertRule(a, b, last) {
  const result = ratiosConvertRuleEx(a, b, last.asPercent);
  if (!areNumbers(b.ratios) || !isNumber2(result.ratio)) {
    throw "ratios does not support non quantity type";
  }
  const index = b.parts.indexOf(a.agent);
...
function compRatiosToCompRuleEx(a, b) {
  if (!areNumbers(a.ratios) || !isNumber2(b.quantity)) {
    throw "ratios does not support non quantity type";
  }
...
function quotaRuleEx(a, quota) {
  ...
  return {
    ...
    entity: a.entity,
    quantity: a.agent === quota.agentQuota ? isNumber2(a.quantity) && isNumber2(quota.quantity) ? a.quantity * quota.quantity : wrapToQuantity(`a.quantity * quota.quantity`, { a, quota }) : isNumber2(a.quantity) && isNumber2(quota.quantity) ? a.quantity / quota.quantity : wrapToQuantity(`a.quantity / quota.quantity`, { a, quota })
  };
}
...
function toPartWholeRatio(part, whole, last) {
    ...
    options: isQuantity && isNumber2(result.quantity) || isRatioPredicate2(result) && isNumber2(result.ratio) ? [
      {
        ...
        result: isQuantity ? isNumber2(result.quantity) ? formatNumber(result.quantity) : "N/A" : isNumber2(result.ratio) ? formatRatio(result.ratio) : "N/A",
        ok: true
      },
      {
        tex: items.map((d) => isQuantity ? formatNumber(d.quantity) : formatRatio(d.ratio)).join(" * "),
...
function toDeltaEx(a, b, last) {
  if (a.entity != b.entity) {
    throw `Mismatch entity ${a.entity}, ${b.entity}`;
  }
  return {
    kind: "delta",
    agent: { name: last.agent?.name ?? b.agent, nameBefore: a.agent, nameAfter: b.agent },
    quantity: isNumber2(a.quantity) && isNumber2(b.quantity) ? b.quantity - a.quantity : wrapToQuantity(`b.quantity - a.quantity`, { a, b }),
    entity: a.entity,
    unit: a.unit
  };
}
function toDelta(a, b, last) {
...
function pythagorasRuleEx(a, b, last) {
  if (a.entity != b.entity) {
    throw `Mismatch entity ${a.entity}, ${b.entity}`;
  }
  if (a.unit != b.unit) {
...
function triangleAngleRuleEx(a, b, last) {
  if (a.entity != b.entity) {
    throw `Mismatch entity ${a.entity}, ${b.entity}`;
  }
  if (a.unit != b.unit) {
...
function solveEquationEx(a, b, last) {
  return {
    kind: "cont",
    agent: last.agent,
...
function solveEquation(a, b, last) {
...
function simplifyExprAsRule(a, b) {
  const result = isQuantityPredicate2(a) ? simplifyExprRuleAsQuantiyEx(a, b) : simplifyExprRuleAsRatioEx(a, b);
  return {
...
function inferenceRuleEx(...args) {
  ...
  if (["sum-combine", "sum", "product-combine", "product", "gcd", "lcd", "sequence", "tuple"].includes(last?.kind) || last?.kind === "ratios" && args.length > 3) {
    ...
    return last.kind === "sequence" ? sequenceRule(arr) : last.kind === "gcd" ? gcdRule(arr, last) : last.kind === "lcd" ? lcdRule(arr, last) : last.kind === "tuple" ? tupleRule(arr) : ["product-combine", "product"].includes(last.kind) ? productRule(arr, last) : ["sum-combine", "sum"].includes(last.kind) ? sumRule(arr, last) : last.kind === "ratios" ? toRatios(arr, last) : null;
    ...
    const kind = last?.kind;
    return kind === "diff" ? toDifferenceAsRatio(a, b, last) : kind === "comp-ratio" ? toComparisonRatio(a, b) : toComparisonAsRatio(a, b);
...
function formatRatio(d, asPercent) {
  ...
  return d > -2 && d < 2 ? helpers2.convertToFraction(d) : formatNumber(d);
}
