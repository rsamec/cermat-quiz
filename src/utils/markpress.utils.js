...
// slightly modified https://github.com/gamell/markpress/blob/master/lib/layout.js
...
function random(params) {
  ...
  const numOfParams = getRandomInt(1, params.length);
  const res = [];
  for (let i = 0; i < numOfParams; i++) {
    // remove 1 elem from index n
    const param = params.splice(getRandomInt(0, params.lenght - 1), 1)[0];
...
function generateNewCoord(lastCoord) {
  let direction = getRandomInt(0, 2); // what direction to move to
