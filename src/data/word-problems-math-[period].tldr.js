...
import { baseDomainPublic, parseCode, normalizeImageUrlsToAbsoluteUrls, text, normalizeLatex, formatShortCode, formatCode } from '../utils/quiz-string-utils.js';
...
//sort codes
codes.sort((f, s) => {
  const first = parseCode(f)
  const second = parseCode(s)
  const diff = parseInt(first.year) - parseInt(second.year);
  return diff == 0 ? first.period.localeCompare(second.period) : diff
})
...
for (let code of codes) {
  ...
  const newPage = createPage({
    name: formatShortCode(code)
  })
  pageRecords.push(newPage)
  ...
  allRecords.push(...resultJson)
}

process.stdout.write(serialize(allRecords));
