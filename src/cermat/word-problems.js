...
function ctorRateQuota(agent, moduloType) {
  return { kind: "quota", agent: normalizeToAgent(agent), moduloType };
}
...
function cont(agent, quantity, entity3, unit, opts) {
  return { kind: "cont", agent: normalizeToAgent(agent), quantity, entity: entity3, unit, asRatio: opts?.asFraction };
}
function tuple(agent, items) {
  return { kind: "tuple", agent, items };
}
function evalExprAsCont(expression, agent, entity3, opts = {}) {
  return { kind: "eval-expr", expression, predicate: { kind: "cont", agent: normalizeToAgent(agent), ...entity3, asRatio: opts.asRatio } };
}
...
function evalFormulaAsCont(f, expression, agent, entity3, opts = {}) {
  return { kind: "eval-formula", expression: expression(f.formula), formulaName: f.name, predicate: { kind: "cont", agent: normalizeToAgent(agent), ...entity3, asRatio: opts.asRatio } };
}
...
function toQuotaRule(a, quota3, last2) {
  const isQuota = isSameEntity(a, quota3);
  const quantity = isNumber(a.quantity) && isNumber(quota3.quantity) ? Math.floor(a.quantity / quota3.quantity) : wrapToQuantity(`floor(a.quantity / quota.quantity)`, { a, quota: quota3 });
  return {
    kind: "quota",
    agent: normalizeToAgent(a.agent),
    entity: isQuota ? last2.entity ?? toEntity("skupin") : toEntity(quota3.entity),
    entityQuota: toEntity(a),
    quantity: isQuota ? quantity : quota3.quantity,
    baseQuantity: 1,
    ...
    moduloType: "floor",
    quotaQuantity: isQuota ? quota3.quantity : quantity
  };
}
function inferToQuotaRule(a, quota3, last2) {
  const result = toQuotaRule(a, quota3, last2);
  const isQuota = isSameEntity(a, quota3);
  if (isNumber(a.quantity) && isNumber(quota3.quantity) && isNumber(result.quantity)) {
    return {
      ...
      question: `Rozd\u011Bl ${formatNumber(a.quantity)} ${formatEntity({ entity: a.entity, unit: a.unit })} postupn\u011B na ${isQuota ? `skupiny velikosti ${formatNumber(quota3.quantity)} ${formatEntity({ entity: quota3.entity, unit: quota3.unit })}` : `${formatNumber(quota3.quantity)} ${formatEntity({ entity: quota3.entity, unit: quota3.unit })}`}`,
      result,
      options: [
...
function inferRestQuantity(quota3, rest) {
  const result = {
    kind: "cont",
    agent: normalizeToAgent(rest.agent ?? quota3.agent),
    entity: quota3.entityQuota.entity,
    unit: quota3.entityQuota.unit,
    quantity: quota3.restQuantity
  };
  return {
    name: "inferRestQuantity",
    inputParameters: extractKinds(quota3, rest),
    question: containerQuestion(result),
    result,
    options: []
  };
}
function inferQuotaQuantity(quota3, rest) {
  const result = {
    kind: "cont",
    agent: normalizeToAgent(rest.agent ?? quota3.agent),
    entity: quota3.entityQuota.entity,
    unit: quota3.entityQuota.unit,
    quantity: quota3.quotaQuantity
  };
  return {
...
function mapToCont({ agent, entity: entity3 }) {
  return (node) => {
    ...
    return {
      ...
      entity: entity3 != null ? entity3.entity : typeNode.kind == "quota" ? typeNode.entityQuota.entity : typeNode.kind == "rate" ? typeNode.entity.entity : typeNode.entity,
      unit: entity3 != null ? entity3.unit : typeNode.kind == "rate" ? typeNode.entity.unit : typeNode.kind == "quota" ? typeNode.entityQuota.unit : typeNode.unit
    };
  };
}
...
function cestaKeStudance() {
  ...
  const tiborMeritko = rate(tiborAgent, 5e4, { entity: entity3, unit }, { entity: entityBase, unit });
  ...
  return {
    ...
    b: {
      deductionTree: deduce(
        ...
        deduce(last(odstrizeno), ctorRestQuantity(["odst\u0159i\u017Een\xED zbytek"])),
...
function hra() {
  ...
  const vypocetPotreba = (ohrada, quota3) => deduce(
    deduce(
      deduceAs("p\u0159\xEDm\xE1 spot\u0159eba")(
        ohrada,
        ohradaPrknaRate
      ),
      deduceAs("spot\u0159eba v\xFDm\u011Bnou za ty\u010De")(
        deduce(
          ohrada,
          ohradaTycRate
        ),
  ...
  const vypocetPotrebaZbytek = (ohrada, quota3, zbytek) => {
    const neprimaSpotreba = deduceAs("spot\u0159eba v\xFDm\u011Bnou za ty\u010De")(
      deduce(
        ohrada,
        ohradaTycRate
      ),
  ...
  return {
    a: {
      deductionTree: vypocetPotreba(cont(agentHra, 3, entityOhrada), ctorRateQuota(void 0, "ceil"))
    },
    b: {
      deductionTree: vypocetPotrebaZbytek(cont(agentHra, 6, entityOhrada), ctorRateQuota(void 0, "ceil"), ctorRestQuantity("nespot\u0159ebovan\xFD zbytek")),
...
// src/math/M7I-2018/index.ts
var M7I_2018_default = createLazyMap({
  11.1: () => kone().a,
  11.2: () => kone().b,
  11.3: () => kone().c
});
function kone() {
  ...
  const dnyEntity = "dny";
  const naseZasoby = "na\u0161e z\xE1soby";
  const naseKone = "na\u0161e kon\u011B";
  const sousedovyZasoby = "sousedovy z\xE1soby";
...
function hranol3() {
  ...
  const vyska = deduce(
    ...
    stranaZakladna,
    evalFormulaAsCont(formulaRegistry.surfaceArea.rectangle, (x) => x.b, "obdeln\xEDk", dim2.length)
  );
  return {
    deductionTree: deduce(
      deduce(
...
function ctyruhelnik() {
  ...
  const CD = deduce(
    ...
    BD,
    evalFormulaAsCont(formulaRegistry.surfaceArea.rectangle, (x) => x.b, "CD", dim2.length)
  );
  return {
    obsah: {
      deductionTree: deduce(
...
function zahon3() {
  ...
  const velikostSkupina = to(
    ...
    cont("velikost skupina(dohromady b\xEDl\xE9 a \u010Derven\xE9 rostliny)", 5, entity3)
  );
  return {
    obvod: {
  ...
  return {
    deductionTree: deduce(
      deduceAs("po\u010D\xEDt\xE1me pouze zv\u011Bt\u0161en\xED/rozd\xEDl mezi dv\u011Bmi hranoly")(
        stranaPovrch,
        strana,
        evalFormulaAsCont(formulaRegistry.surfaceArea.rectangle, (x) => x.b, "jedna bo\u010Dn\xED st\u011Bna", dim2.length)
      ),
      ctorOption("B", 6)
    )
  };
...
function vypocet() {
  return {
    deductionTree: deduce(
      deduce(
        counter("prvn\xED prvo\u010D\xEDslo", 2),
        counter("druh\xE9 prvo\u010D\xEDslo", 3),
        counter("t\u0159et\xED prvo\u010D\xEDslo", 5),
        sum("sou\u010Det")
      ),
...
function houskyARohliky() {
  ...
  const rohlikLabel = "rohl\xEDk";
  const entity3 = "kus";
  const cena = "korun";
  const houskaRate = rate(houskaLabel, "h", cena, entity3);
  const triRohliky = deduce(
    deduce(
      houskaRate,
      cont(houskaLabel, 2, entity3)
    ),
    compDiff(houskaLabel, rohlikLabel, 6, cena)
  );
  const rohlikRate = deduce(
    last(triRohliky),
    cont(rohlikLabel, 3, entity3),
    ctor("rate")
  );
  return {
    rohlik: {
    ...
    cenaHousky: {
      deductionTree: deduce(
        deduce(
          deduce(
            cont(["Kl\xE1ra", houskaLabel], 6, entity3),
            houskaRate
          ),
          deduce(
            cont(["Kl\xE1ra", rohlikLabel], 6, entity3),
            last(rohlikRate)
          ),
          sum("Kl\xE1ra zaplaceno")
        ),
        cont("Kl\xE1ra zaplaceno", 78, cena),
...
function automat() {
  ...
  const umeraDobaAutomat = proportion(true, [entity3, entityAutomat]);
  const zakazka = "cel\xE1 zak\xE1zka";
  const dobaNoveZakazky = (noveAutomat, targetAgent) => deduce(
    ...
    cont(targetAgent, 60, entity3, unit)
  );
  const prvniCastZakazky = ratio(zakazka, "prvn\xED \u010D\xE1st", 1 / 4);
  const potrebaEntity = "pot\u0159eba";
  return {
    pomer: {
      deductionTree: deduce(
        cont(["nov\u011B", "skute\u010Dn\u011B"], 24, entity3),
      ...
      deductionTree: deduce(
        deduce(
          deduce(
            ...
            comp("Adam", "Ben", -45, { entity: entity3, unit })
          ),
  ...
  return {
    deductionTree: deduce(
      deduce(
        kratsi,
        delsi,
        evalFormulaAsCont(formulaRegistry.surfaceArea.triangle, (x) => x.S, "troj\xFAheln\xEDk ABC", dim2.area)
      ),
      ctorOption("A", 96)
    )
  };
    ...
    deductionTree: deduce(
      deduce(
        deduce(
          ...
          contLength(["hranol", "strana"], 2),
          contLength(["hranol", "v\xFD\u0161ka"], 4),
          evalFormulaAsCont(formulaRegistry.volume.cuboid, (x) => x.V, "hranol", dim2.volume)
        ),
        deduce(
          deduce(
            contLength(["v\xE1lec", "polom\u011Br"], 1),
            contLength(["v\xE1lec", "v\xFD\u0161ka"], 4),
            evalFormulaAsCont(formulaRegistry.volume.cylinder, (x) => x.V, "v\xE1lec", dim2.volume)
          ),
          ...halfProduct("polovina v\xE1lce")
        ),
        ctorDifference("t\u011Bleso s prohlubn\xED")
      ),
      ctorOption("B", 9.72)
    )
  };
...
function uhelAlfa2() {
  ...
  return {
    deductionTree: deduce(
      deduce(
        deduceAs("Vyu\u017Eit\xED vlastnost\xED \xFAhlop\u0159\xED\u010Dek koso\u010Dtverce")(
          contRightAngle("BSC"),
          deduceAs("Vlastnost t\u011B\u017Enice v pravo\xFAhl\xE9m troj\xFAheln\xEDku, resp. troj\xFAheln\xEDk CSX je rovnoramenn\xFD s rameny SX and CX")(
            csx,
...
function trojuhelnik3() {
  ...
  const delsiLabel = "del\u0161\xED strana";
  const agent = "troj\xFAheln\xEDk";
  const entityBase = "krok";
  const kratsi = contLength(kratsiLabel, 5);
  const delsi = contLength(delsiLabel, 12);
  const vychoziRozdilDelek = deduce(delsi, kratsi, ctorDifference("v\xFDchoz\xED rozd\xEDl d\xE9lek odv\u011Bsen"));
  const delsiStranaRate = rate(delsiLabel, 2, dim2.length, entityBase);
  const kratsiStranaRate = rate(kratsiLabel, 5, dim2.length, entityBase, 6);
  const rateRozdilDelek = rate(rozdilDelekLabel, 7, dim2.length, entityBase, 6);
    ...
    b: {
      deductionTree: deduce(
        deduce(
          ...
          contLength(`v\xFDchoz\xED ${delsiLabel}`, 12),
          ctor("slide")
        ),
        pomerStran,
        nthPart(kratsiLabel)
      ...
      deductionTree: deduce(
        deduce(
          deduce(
            ...
            contLength(`v\xFDchoz\xED ${kratsiLabel}`, 5),
            ctor("slide-invert")
          ),
          kratsiStranaRate
        ),
...
function procenta11() {
  ...
  const syrMensiBaleniLabel = "men\u0161\xED balen\xED";
  const syrVetsiBaleniLabel = "v\u011Bt\u0161\xED balen\xED";
  const lodeEntity = "lo\u010F";
  const dnyEntity = "den";
  const lodeDnyEntity = "lodPerDen";
  const lyze = "ly\u017Ee";
  const modelLoniLabel = "lo\u0148sk\xFD model";
  const modelLetosLabel = "leto\u0161n\xED model";
  const nasetrenoLabel = "na\u0161et\u0159eno";
  const modelLoni = cont(modelLoniLabel, 1e4, entityCena, unit);
  ...
  return {
    ...
    druha: {
      deductionTree: deduce(
        deduce(
          deduce(
            cont("vybran\xE9 pen\xEDze", 2, lodeEntity),
            cont("vybran\xE9 pen\xEDze", 4, dnyEntity),
            productCombine("vybran\xE9 pen\xEDze", lodeDnyEntity)
          ),
          deduce(
            cont("vybran\xE9 pen\xEDze", 10, lodeEntity),
            cont("vybran\xE9 pen\xEDze", 5, dnyEntity),
            productCombine("vybran\xE9 pen\xEDze", lodeDnyEntity)
          ),
          ctorPercent()
        ),
