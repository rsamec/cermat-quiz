...
import wordProblems, { inferenceRuleWithQuestion, formatPredicate, formatSequencePattern } from './word-problems.js';
  ...
  formatQuantity: d => {
     if (typeof d === "number") {
       return d.toLocaleString("cs-CZ");
        ...
        return `<div class="badge badge--warning">${toEquationExprAsText(d)}</div>`
     }
...
<div>${q?.question}</div>
${answer != null ? `<div>${mdPlus.renderToString(`$${answer.tex} = ${answer.result}$`)}</div>` : ''}
</div>`: ''}
