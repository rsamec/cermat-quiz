...
```js
import { parseCode, formatSubject, formatPeriod, formatShortCode } from '../utils/quiz-string-utils.js';
...
const parsedCodes = Object.entries(Object.groupBy(folders.map(d => parseCode(d)).filter(d => d.subject === subject), d => `${formatPeriod(d.period)}`));
...
<div class="grid grid-cols-4" style="grid-auto-rows: auto;">
 ${parsedCodes.map(([groupKey, codes]) => html`<div class="card">
    ...
    <div class="v-stack v-stack--s">
      ...
      <div class="v-stack v-stack--l">
        ${codes.sort((f,s) => s.year - f.year).map(({code, subject, year}) => html`<div class="h-stack h-stack--m h-stack-items--center h-stack--wrap">
              <a class="h-stack h-stack--s" href="./solution-${code}"><i class="fas fa-money-check"></i><span>${formatShortCode(code)}</span></a>
...
${html`Balíček <b>všech zadání úloh</b> ve formátu markdown členěno dle granularity až na úroveň<ul><li><a download class="h-stack h-stack--s  h-stack-items--center" href="/cermat/question-${subject}.zip"><i class="fa fa-brands fa-markdown"></i><span>jednotlivých testů</span></a></li><li><a download class="h-stack h-stack--s  h-stack-items--center" href="/cermat/questions-${subject}.zip"><i class="fa fa-brands fa-markdown"></i><span>jednotlivých úloh</span></a></li></ul>`}


${ subject == 'math' ? html`Balíček <b>úloh s postupy řešení</b> ve formátu markdown členěno dle granularity až na úroveň<ul><li><a download class="h-stack h-stack--s  h-stack-items--center" href="/cermat/word-problems.zip"><i class="fa fa-brands fa-markdown"></i><span>jednotlivých testů</span></a></li><li><a download class="h-stack h-stack--s  h-stack-items--center" href="/cermat/word-problem.zip"><i class="fa fa-brands fa-markdown"></i><span>jednotlivých úloh</span></a></li></ul>`:'' }


## Balíčky PDF pro tisk ke stažení

```js
const pdfs = await FileAttachment(`./pdf-${observable.params.subject}.json`).json();
...
<ul>${Object.entries(pdfs).filter(([key]) => parseFileKey(key).pageSize === pageSize && parseFileKey(key).pageOrientation === pageOrientation).map(([key, values]) => html`<li><a href="/assets/pdf/cermat-${observable.params.subject}/${key}.pdf"><i class="fa-solid fa-file-pdf"></i> ${key} (${values.reduce((out,[v,n])=> out+=n,0)} stránek)</a></li>`)}</ul>
