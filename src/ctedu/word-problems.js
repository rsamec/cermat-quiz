...
function isQuantityPredicate(value) {
  return ["cont", "comp", "transfer", "rate", "comp-diff", "transfer", "rate-quota", "quota", "delta", "frequency"].includes(value.kind);
}
...
function ctorRestQuantity(agent) {
  return { kind: "rest-quantity", agent: normalizeToAgent(agent) };
}
function ctorQuotaQuantity(agent) {
  return { kind: "quota-quantity", agent: normalizeToAgent(agent) };
}
function ctorQuota(agent, entity, moduloType) {
  return { kind: "quota", agent: normalizeToAgent(agent), entity, moduloType };
}
...
function comp(agentA, agentB, quantity, entity) {
...
var rightAngleAgent = "prav\xFD \xFAhel";
function contRightAngle(agent) {
  return contAngle(agent ?? rightAngleAgent, 90, "deg");
}
var triangleAngleSumAgent = "sou\u010Det \xFAhl\u016F v troj\xFAheln\xEDku";
...
function gcd(agent, entity, unit) {
...
function inferInvertToRule(a, b) {
  const result = {
    ...a,
    quantity: isNumber(a.quantity) ? a.quantity * -1 : wrapToQuantity(`a.quantity * -1`, { a })
  };
  return {
    name: "invertRule",
    inputParameters: extractKinds(a, b),
    question: isNumber(a.quantity) ? `P\u0159eve\u010F ${formatNumber(a.quantity)} ${formatEntity(a)} na opa\u010Dnou hodnotu.` : `P\u0159eve\u010F na opa\u010Dnou hodnotu.`,
    result,
    options: isNumber(a.quantity) && isNumber(result.quantity) ? [] : []
  };
}
  ...
  return {
    ...
    options: isNumber(a.quantity) && isNumber(b.ratio) && isNumber(result.quantity) ? [
      ...
      },
      {
        ...
        ok: ![a.agent, a.entity].flat().includes(b.agentA) && ![a.agent, a.entity].includes(b.agentB) && b.agentA !== nthPart2?.agent
      },
      {
        tex: `${formatNumber(a.quantity)} / (${formatRatio(abs(b.ratio))} + 1) * ${formatRatio(abs(b.ratio))}`,
        result: formatNumber(result.quantity),
        ok: b.agentA == nthPart2?.agent
      }
    ] : []
  };
}
...
function rateRule(a, rate2) {
  const aEntity = a.kind == "cont" ? toEntity(a) : a.kind === "quota" ? a.entityQuota : a.entity;
  if (!(isSameEntity(aEntity, rate2.entity) || isSameEntity(aEntity, rate2.entityBase))) {
    throw `Mismatch entity ${formatEntity(aEntity)} any of ${formatEntity(rate2.entity)}, ${formatEntity(rate2.entityBase)}`;
  }
  const isEntityBase2 = isSameEntity(aEntity, rate2.entity);
  ...
  return {
    ...
    quantity: isEntityBase2 ? isNumber(a.quantity) && isNumber(rate2.quantity) && isNumber(rate2.baseQuantity) ? a.quantity / (!isUnitRate ? rate2.quantity / rate2.baseQuantity : rate2.quantity) : !isUnitRate ? wrapToQuantity(`a.quantity / (rate.quantity/rate.baseQuantity)`, { a, rate: rate2 }) : wrapToQuantity(`a.quantity / rate.quantity`, { a, rate: rate2 }) : isNumber(a.quantity) && isNumber(rate2.quantity) && isNumber(rate2.baseQuantity) ? a.quantity * (!isUnitRate ? rate2.quantity / rate2.baseQuantity : rate2.quantity) : !isUnitRate ? wrapToQuantity(`a.quantity * (rate.quantity/rate.baseQuantity)`, { a, rate: rate2 }) : wrapToQuantity(`a.quantity * rate.quantity`, { a, rate: rate2 })
  };
}
function inferRateRule(a, rate2) {
  const result = rateRule(a, rate2);
  const aEntity = a.kind == "cont" ? a.entity : a.kind === "quota" ? a.agent : a.entity.entity;
  const isUnitRate = rate2.baseQuantity === 1;
  return {
    name: rateRule.name,
...
function toQuotaRule(a, quota2, last2) {
  const isQuota = isSameEntity(a, quota2);
  const quantity = isNumber(a.quantity) && isNumber(quota2.quantity) ? Math.floor(a.quantity / quota2.quantity) : wrapToQuantity(`floor(a.quantity / quota.quantity)`, { a, quota: quota2 });
  return {
    kind: "quota",
    agent: normalizeToAgent(a.agent),
    entity: isQuota ? last2.entity ?? toEntity("skupin") : toEntity(quota2.entity),
    entityQuota: toEntity(a),
    quantity: isQuota ? quantity : quota2.quantity,
    baseQuantity: 1,
    ...
    moduloType: "floor",
    quotaQuantity: isQuota ? quota2.quantity : quantity
  };
}
function inferToQuotaRule(a, quota2, last2) {
  const result = toQuotaRule(a, quota2, last2);
  const isQuota = isSameEntity(a, quota2);
  if (isNumber(a.quantity) && isNumber(quota2.quantity) && isNumber(result.quantity)) {
    return {
      ...
      question: `Rozd\u011Bl ${formatNumber(a.quantity)} ${formatEntity({ entity: a.entity, unit: a.unit })} postupn\u011B na ${isQuota ? `skupiny velikosti ${formatNumber(quota2.quantity)} ${formatEntity({ entity: quota2.entity, unit: quota2.unit })}` : `${formatNumber(quota2.quantity)} ${formatEntity({ entity: quota2.entity, unit: quota2.unit })}`}`,
      result,
      options: [
...
function rateQuotaRule(a, rate2, moduloType = "floor") {
  const aEntity = a.entity;
  if (!(aEntity === rate2.entity.entity)) {
  ...
  if (moduloType === "ceil") {
    quantities = {
      quantity: isNumber(a.quantity) && isNumber(rate2.quantity) && isNumber(rate2.baseQuantity) ? isUnitRate ? Math.ceil(a.quantity / rate2.quantity) : Math.ceil(a.quantity / rate2.quantity) * rate2.baseQuantity : isUnitRate ? wrapToQuantity(`ceil(a.quantity / rate.quantity)`, { a, rate: rate2 }) : wrapToQuantity(`ceil(a.quantity / rate.quantity) * rate.baseQuantity`, { a, rate: rate2 }),
      restQuantity: isNumber(a.quantity) && isNumber(rate2.quantity) ? a.quantity - rate2.quantity * Math.ceil(a.quantity / rate2.quantity) : wrapToQuantity(`a.quantity - rate.quantity * ceil(a.quantity / rate.quantity)`, { a, rate: rate2 })
    };
  } else if (moduloType === "floor") {
    quantities = {
      quantity: isNumber(a.quantity) && isNumber(rate2.quantity) && isNumber(rate2.baseQuantity) ? isUnitRate ? Math.floor(a.quantity / rate2.quantity) : Math.floor(a.quantity / rate2.quantity) * rate2.baseQuantity : isUnitRate ? wrapToQuantity(`floor(a.quantity / rate.quantity)`, { a, rate: rate2 }) : wrapToQuantity(`floor(a.quantity / rate.quantity) * rate.baseQuantity`, { a, rate: rate2 }),
      restQuantity: isNumber(a.quantity) && isNumber(rate2.quantity) ? a.quantity - rate2.quantity * Math.floor(a.quantity / rate2.quantity) : wrapToQuantity(`a.quantity - rate.quantity * floor(a.quantity / rate.quantity)`, { a, rate: rate2 })
    };
  } else {
    throw `Not supported moduloType:${moduloType}`;
  }
  return {
    kind: "quota",
    agent: normalizeToAgent(a.agent),
    entity: rate2.entityBase,
    entityQuota: rate2.entity,
    quotaQuantity: rate2.quantity,
    quantity: quantities.quantity,
    baseQuantity: rate2.baseQuantity,
    restQuantity: quantities.restQuantity,
    moduloType
  };
}
function inferRateQuotaRule(a, rate2, last2) {
  const result = rateQuotaRule(a, rate2, last2?.moduloType);
  const aEntity = a.entity;
  const isUnitRate = rate2.baseQuantity === 1;
  return {
    name: rateRule.name,
    inputParameters: extractKinds(a, rate2),
    question: isNumber(a.quantity) && isNumber(rate2.quantity) && isNumber(result.quantity) ? `Rozd\u011Bl ${formatNumber(a.quantity)} ${formatEntity({ entity: a.entity, unit: a.unit })} postupn\u011B na skupiny velikosti ${formatNumber(rate2.quantity)} ${formatEntity(rate2.entity)}` : "",
    result,
    options: isNumber(a.quantity) && isNumber(rate2.quantity) && isNumber(result.quantity) && isNumber(rate2.baseQuantity) ? [
      { tex: `floor(${formatNumber(a.quantity)} * ${formatNumber(rate2.quantity)})`, result: formatNumber(result.quantity), ok: isUnitRate && aEntity !== rate2.entity.entity },
      ...!isUnitRate ? [{ tex: `floor(${formatNumber(a.quantity)} / ${formatNumber(rate2.baseQuantity)}) * ${formatNumber(rate2.quantity)}`, result: formatNumber(result.quantity), ok: !isUnitRate && aEntity !== rate2.entity.entity }] : [],
      { tex: `ceil(${formatNumber(a.quantity)} / ${formatNumber(rate2.quantity)})`, result: formatNumber(result.quantity), ok: isUnitRate && aEntity === rate2.entity.entity },
      ...!isUnitRate ? [{ tex: `ceil(${formatNumber(a.quantity)} / ${formatNumber(rate2.quantity)}) * ${formatNumber(rate2.baseQuantity)}`, result: formatNumber(result.quantity), ok: !isUnitRate && aEntity === rate2.entity.entity }] : []
    ] : []
  };
}
function inferRestQuantity(quota2, rest) {
  const result = {
    kind: "cont",
    agent: normalizeToAgent(rest.agent ?? quota2.agent),
    entity: quota2.entityQuota.entity,
    unit: quota2.entityQuota.unit,
    quantity: quota2.restQuantity
  };
  return {
    name: "inferRestQuantity",
    inputParameters: extractKinds(quota2, rest),
    question: containerQuestion(result),
    result,
    options: []
  };
}
function inferQuotaQuantity(quota2, rest) {
  const result = {
    kind: "cont",
    agent: normalizeToAgent(rest.agent ?? quota2.agent),
    entity: quota2.entityQuota.entity,
    unit: quota2.entityQuota.unit,
    quantity: quota2.quotaQuantity
  };
  return {
    name: "inferQuotaQuantity",
    inputParameters: extractKinds(quota2, rest),
    question: containerQuestion(result),
    result,
    options: []
...
function toPartWholeRatio(part, whole, asPercent) {
  const toAgent = (predicate) => {
    ...
    const entities = predicate.kind == "rate" ? [predicate.entityBase.entity] : predicate.kind == "quota" ? [predicate.entityQuota.entity] : [];
...
function toCompareRule(a, b) {
  const aEntity = a.kind === "rate" ? a.entity : a.kind === "quota" ? a.entityQuota : { entity: a.entity, unit: a.unit };
  const bEntity = b.kind === "rate" ? b.entity : b.kind === "quota" ? b.entityQuota : { entity: b.entity, unit: b.unit };
...
function toRateRule(a, b, rate2) {
  ...
  return {
    ...
    entityBase: {
      entity: b.kind === "cont" ? b.entity : b.entityQuota.entity,
      unit: b.kind === "cont" ? b.unit : b.entityQuota.unit
    },
...
function inferToRateRule(a, b, rate2) {
  ...
  if (isNumber(a.quantity) && isNumber(b.quantity) && isNumber(result.baseQuantity) && isNumber(result.quantity)) {
    return {
      ...
      question: `Rozd\u011Bl ${formatNumber(a.quantity)} ${formatEntity({ entity: a.entity })} rovnom\u011Brn\u011B ${formatNumber(b.quantity)} kr\xE1t${result.baseQuantity !== 1 ? ` po ${formatNumber(result.baseQuantity)} ${formatEntity(b.kind === "cont" ? toEntity(b) : b.entityQuota)}` : ""}`,
      result,
      options: [
...
function partToPartRule(a, partToPartRatio, nth) {
  const aEntity = a.kind == "quota" ? { entity: a.agent } : toEntity(a.entity);
...
function inferenceRuleEx(...args) {
  ...
  if (["sum-combine", "sum", "product-combine", "product", "gcd", "lcd", "sequence", "tuple", "eval-expr", "eval-formula", "alligation"].includes(last2?.kind)) {
    ...
    if (kind === "quota")
      return inferToQuotaRule(a, b, last2);
    if (kind === "delta")
  ...
  } else if (a.kind === "cont" && b.kind === "invert") {
    return inferInvertToRule(a, b);
  } else if (a.kind === "invert" && b.kind === "cont") {
    return inferInvertToRule(b, a);
    ...
    return kind === "ratio" ? inferToPartWholeRatio(b, a, last2) : kind === "quota" && a.kind === "cont" ? inferRateQuotaRule(a, b, last2) : inferRateRule(a, b);
  } else if (a.kind === "rate" && (b.kind == "cont" || b.kind === "quota" || b.kind === "rate")) {
    return kind === "ratio" ? inferToPartWholeRatio(a, b, last2) : kind === "quota" && b.kind === "cont" ? inferRateQuotaRule(b, a, last2) : inferRateRule(b, a);
  ...
  } else if (a.kind === "cont" && b.kind == "quota" && kind == "rate") {
    return inferToRateRule(a, b, last2);
  } else if (a.kind === "quota" && b.kind == "cont" && kind == "rate") {
    return inferToRateRule(b, a, last2);
  } else if (a.kind === "rest-quantity" && b.kind == "quota") {
    return inferRestQuantity(b, a);
  } else if (a.kind === "quota" && b.kind == "rest-quantity") {
    return inferRestQuantity(a, b);
  } else if (a.kind === "quota-quantity" && b.kind == "quota") {
    return inferQuotaQuantity(b, a);
  } else if (a.kind === "quota" && b.kind == "quota-quantity") {
    return inferQuotaQuantity(a, b);
...
function mapToCont({ agent, entity }) {
  return (node) => {
    ...
    return {
      ...
      entity: entity != null ? entity.entity : typeNode.kind == "quota" ? typeNode.entityQuota.entity : typeNode.kind == "rate" ? typeNode.entity.entity : typeNode.entity,
      unit: entity != null ? entity.unit : typeNode.kind == "rate" ? typeNode.entity.unit : typeNode.kind == "quota" ? typeNode.entityQuota.unit : typeNode.unit
    };
  };
}
...
function formatPredicate(d, formatting) {
  ...
  switch (d.kind) {
    ...
    case "rate":
      ...
      break;
    case "quota":
      result = d.entityQuota == null ? formatKind(d) : compose`${formatAgent(d.agent)} ${formatQuantity(d.quantity)} ${formatEntity2(d.entity.entity, d.entity.unit)} ${isNumber(d.baseQuantity) && d.baseQuantity == 1 ? "po " : `pro ka\u017Ed\xE9 ${formatQuantity(d.baseQuantity)} ${formatEntity2(d.entity.entity, d.entity.unit)} po `} ${formatQuantity(d.quotaQuantity)}${d.entityQuota.entity != "" ? " " : ""}${formatEntity2(d.entityQuota.entity, d.entityQuota.unit)}
         ${isNumber(d.restQuantity) ? d.restQuantity !== 0 ? ` se zbytkem ${formatQuantity(d.restQuantity)}${d.entityQuota.entity != "" ? " " : ""}${formatEntity2(d.entityQuota.entity, d.entityQuota.unit)}` : "" : `se zbytkem ${formatQuantity(d.restQuantity)}${d.entityQuota.entity != "" ? " " : ""}${formatEntity2(d.entityQuota.entity, d.entityQuota.unit)}`}`;
      break;
    case "sequence":
...
// src/ctedu/2026-01-06/index.ts
var __default = createLazyMap({
  1: () => porovnani(),
  3.1: () => rovnosti().prvni,
  3.2: () => rovnosti().druha,
  3.3: () => rovnosti().treti,
...
function porovnani() {
  return {
    deductionTree: deduce(
      deduce(
        counter("prvn\xED zadan\xE9 \u010D\xEDslo", 7),
        counter("\u010Dty\u0159n\xE1sobek", 4),
        product("prvn\xED zadan\xE9 \u010D\xEDslo")
      ),
      counter("druh\xE9 zadan\xE9 \u010D\xEDslo", 140),
      ctor("comp-ratio")
    ),
...
function rovnosti() {
  ...
  return {
    prvni: {
      deductionTree: deduce(
        deduce(
          contArea("lev\xE1 strana", 0.45, "m2"),
          ctorUnit("cm2")
        ),
        contArea("prav\xE1 strana", 20),
    ...
    druha: {
      deductionTree: deduce(
        deduce(
          contVolume("prav\xE1 strana", 2, "l"),
          ctorUnit("cm3")
        ),
        deduce(
          contVolume("lev\xE1 strana", 0.8, "dm3"),
          ctorUnit("cm3")
        ),
        ctorDifference("zbytek do rovnosti na lev\xE9 stran\u011B")
      )
    },
    treti: {
      deductionTree: deduce(
        deduce(
          deduce(
            cont("prav\xE1 strana", 0.75, entityTime, "h"),
            ctorUnit("min")
          ),
          counter("desetin\xE1sobek", 10),
          product("celkem prav\xE1 strana")
        ),
        cont("lev\xE1 strana", 30, entityTime, "min"),
        ctor("quota")
      )
    }
  };
    ...
    deductionTree: deduce(
      deduce(
        deduce(
          SD,
          AC,
          evalFormulaAsCont(formulaRegistry.surfaceArea.triangle, (x) => x.S, "troj\xFAheln\xEDk ACD", dim2.area)
        ),
        ...doubleProduct("obdeln\xEDk ABCD")
      ),
      ctorOption("D", 240)
    )
  };
...
function procenta() {
  ...
  const unit = "K\u010D";
  const tabor3 = cont("t\xE1bor", 2400, entityCena, unit);
  return {
    prvni: {
      deductionTree: deduce(
        deduce(
          deduce(
            cont("ubytov\xE1no ve stanech", 120, entityDeti),
            cont("t\xE1bor celkem", 160, entityDeti),
            ctorPercent()
          ),
          ctorComplement("nebylo ubytov\xE1no ve stanech")
        ),
    ...
    druha: {
      deductionTree: deduce(
        deduce(
          deduce(
            compRelative("Petr", "Luk\xE1\u0161", 1 / 4),
        ...
        deduce(
          deduce(
            deduce(
              deduce(
                tabor3,
                ratio("t\xE1bor", "doprava", 1 / 5)
              ),
              cont("ubytov\xE1n\xED", 1080, entityCena, unit),
              sum("doprava a ubytov\xE1n\xED")
            ),
            tabor3,
            ctorPercent()
          ),
          ctorComplement("stravov\xE1n\xED")
        ),
...
// src/ctedu/2026-01-13/index.ts
var __default2 = createLazyMap({
  4: () => obyvatelSporty(),
  5: () => velikostUhlu(),
  6.1: () => procenta2().treti,
  6.2: () => procenta2().druhy,
  6.3: () => procenta2().druhyPocet
});
...
function procenta2() {
  const entity = "testy";
  const prvni = "prvn\xED den";
  const druhy = "druh\xFD den";
  const treti = "posledn\xED den";
    ...
    deductionTree: deduce(
      deduce(
        deduce(
          ...
          cont("obd\xE9ln\xEDk", 20, ctverecLabel),
          ctor("rate")
        ),
...
function vypocet2() {
  return {
    deductionTree: deduce(
      contAngle("1. \xFAhel", 25, "arcmin"),
      deduce(
        contAngle("2. \xFAhel", 5),
        ctorUnit("arcmin")
      ),
      ctor("comp-ratio")
    ),
...
function firma() {
  const entity = "osob";
  const zeny = cont(["firma", "zam\u011Bstnan\xE9 \u017Eeny"], "x", entity);
  const celkem = cont(["firma", "zam\u011Bstn\xE1no celkem"], 200, entity);
  const zenyNaPracovisti = deduce(
    ratio("zam\u011Bstnan\xE9 \u017Eeny", "v\u010Dera na pracovi\u0161ti", 1 / 3),
    zeny
  );
  const muziNaPracovisti = deduce(
    ratio("zam\u011Bstnan\xED mu\u017Ei", "v\u010Dera na pracovi\u0161ti", 2 / 5),
    deduce(
      celkem,
      zeny,
      ctorDifference("zam\u011Bstnan\xED mu\u017Ei")
    )
  );
  return {
    zeny: {
    ...
    celkemZen: {
      deductionTree: deduce(
        ...
        ctorLinearEquation("zam\u011Bstnan\xE9 \u017Eeny", { entity }, "x")
      )
    }
  };
}
function plaveckyBazen() {
  const entity = "dosp\u011Bl\xFDch";
  return {
    deductionTree: deduce(
      deduce(
...
function problemy() {
  return {
    deductionTree: deduce(
      deduce(
        deduce(
          compRelative("Kl\xE1ra", "Petr", -1 / 5),
          ctor("invert-comp-ratio")
        ),
        ctor("convert-percent")
      ),
...
function pozemek() {
  const pozemek2 = contLength("c", 30, "m");
...
function zahon() {
  ...
  const rozestup = rate("rostliny", 40, dim2.length, entity);
  const obvod = deduce(
    pocetRostlin,
    rozestup
  );
  ...
  const velikostSkupina = to(
    ...
    commonSense(`kombinace 5x13 nebo 13x5, vybereme v\u011Bt\u0161\xED po\u010Det opakov\xE1n\xED a men\u0161\xED skupinu, aby jsme doc\xEDlili men\u0161\xEDho po\u010Dtu \u010Derven\xFDch r\u016F\u017E\xED`),
    cont("velikost skupina(dohromady b\xEDl\xE9 a \u010Derven\xE9 rostliny)", 5, entity)
  );
  return {
    obvod: {
    ...
    nejmensiPocetCerveneKvetoucich: {
      deductionTree: deduce(
        deduce(
          velikostSkupina,
          cont("b\xEDl\xFDch rostlin", 2, entity),
...
function pelhrimov() {
  ...
  const prihlasek = cont("t\xE1bor p\u0159ihl\xE1\u0161eno", 375, entity);
      ...
      deduce(
        deduce(
          ...
          deduce(
            ...
            rate("hr\xE1\u010Di", 11, entity, entityBase)
          ),
  ...
  return {
    deductionTree: deduce(
      deduce(
        ...
        rate("aritmetick\xFD pr\u016Fm\u011Br", 1.8, entity, entityPocet),
...
function uhelAlfa() {
  ...
  const praveRameho = deduce(
    ratios(triangleAngleSumAgent, ["vrcholov\xFD \xFAhel", "lev\xE9 rameno", "prav\xE9 rameno"], [40, 70, 70]),
...
function tabor() {
  ...
  const deti = cont("t\xE1bor", 80, entity);
      ...
      deductionTree: deduce(
        deduce(
          deduce(
            ...
            ctorLinearEquation("po\u010Det d\xEDvek", { entity }, "x")
          ),
          deti,
          ctorPercent()
        ),
...
function porovnani4() {
  ...
  const zadane1 = cont("zadan\xE9 mno\u017Estv\xED", 5, entity, "kg");
  const zadane2 = cont("zadan\xE9 mno\u017Estv\xED", 0.25, entity, "g");
  return {
    deductionTree: deduce(
      deduce(
        zadane1,
...
function krychle() {
  const dim2 = dimensionEntity();
  const agent = "krychle";
  const entity = "b\xEDl\xFD \u010Dtvere\u010Dek";
...
function mozaika() {
  ...
  const entityRadek = "\u0159\xE1dk\u016F";
  const obrazec12 = "mozaika s 12 \u0159adami";
  const korekce = "korekce okraje";
  const sedeCtverce = "\u0161ed\xE9 \u010Dtverce";
  const bileCtverce = "b\xEDl\xE9 \u010Dtverce";
  const ctverec2 = "\u010Dtverec";
  ...
  const kratsiStrana = deduce(
    ...
    ctorQuotaQuantity("krat\u0161\xED strana")
  );
  return {
    a: {
      deductionTree: deduce(
    ...
    b: {
      deductionTree: deduce(
        ...
        deduce(
          last(kratsiStrana),
          deduceAs("2 zbyl\xE9 b\xEDl\xE9 \u010Dtverce odpov\xEDd\xE1j\xED 1 + 1 pro del\u0161\xED stranu")(
            deduce(
              last(kratsiStranaQuota),
    ...
    c: {
      deductionTree: deduce(
        cont([mozajka, "celkem"], 380, ctverec2),
        deduceAs("Hled\xE1me dv\u011B \u010D\xEDsla vedle sebe (\u0159ady a sloupce), jejich\u017E sou\u010Din je 380.")(
          deduce(
          ...
          productCombine([mozajka, sedeCtverce], { entity: ctverec2 })
        ),
        ctorDifference(mozajka)
      )
    }
...
function symboly() {
  ...
  const plusAgent = "lich\xE9";
  const kratAgent = "sud\xE9";
  ...
  const pipnuti11 = "obrazovka p\u0159i 11. p\xEDpnut\xED";
  const pipnuti90 = "obrazovka p\u0159i 90. p\xEDpnut\xED";
...
// src/ctedu/2026-03-17/index.ts
var __default11 = createLazyMap({
  1: () => vypocet3(),
  4.1: () => les().zajici,
  4.2: () => les().divocaci,
  5.1: () => vylet().a,
...
function vypocet3() {
  return {
    deductionTree: deduce(
      contAngle("1. \xFAhel", 12, "arcmin"),
      deduce(
        deduce(
          contAngle("2. \xFAhel", 6),
          ctorUnit("arcmin")
        ),
        contAngle("2. \xFAhel", 24, "arcmin"),
        sum("2. \xFAhel")
      ),
      ctor("comp-ratio")
    ),
...
function les() {
  const entityPrase = "divo\u010D\xE1k";
  const entityZajic = "zaj\xEDc";
  const predSezonouPrase = cont(["p\u0159ed sez\xF3nou"], "x", entityPrase);
    ...
    divocaci: {
      deductionTree: deduce(
        deduce(
          ...
          ctorLinearEquation("p\u0159ed sez\xF3nou", { entity: entityPrase }, "x")
        ),
        zastrelono,
        ctorDifference("po sez\xF3n\u011B")
      )
...
function vylet() {
  ...
  const doba = cont("celkem doba v\xFDletu", 291, entityMinutes.entity, entityMinutes.unit);
  const zdrzeni = cont("zdr\u017Een\xED", 90, entityMinutes.entity, entityMinutes.unit);
  const cistaDoba = deduce(
    doba,
    zdrzeni,
    ctorDifference("doba cesty")
  );
  const cestaParts = ratios("doba cesty", ["cesta do cukr\xE1rny", "cesta zp\u011Bt"], [2, 1]);
  const cestaTam = deduce(
    cistaDoba,
    cestaParts,
    nthPart("cesta do cukr\xE1rny")
  );
    ...
    c: {
      deductionTree: deduce(
        deduce(
          ...
          evalFormulaAsCont(formulaRegistry.speed, (x) => x.v, "pr\u016Fm\u011Brn\xE1 rychlost", { entity: "rychlost", unit: "km/h" })
        ),
  ...
  return {
    deductionTree: deduce(
      deduce(
        ...
        evalFormulaAsCont(formulaRegistry.surfaceArea.square, (x) => x.S, "obsah", dim2.area)
      ),
      ctorOption("D", 144)
    )
  };
...
// src/ctedu/2026-03-24/index.ts
var __default12 = createLazyMap({
  1: () => obdelnik3(),
  3.1: () => kinoSal().a,
  3.2: () => kinoSal().b,
  3.3: () => kinoSal().c,
...
function obdelnik3() {
  const dim2 = dimensionEntity();
  const delka = contLength("d\xE9lka", 15);
  return {
    deductionTree: deduce(
      delka,
      deduce(
        deduce(
          contArea("obd\xE9ln\xEDk", 1.05, "dm2"),
          ctorUnit("cm2")
        ),
        delka,
        evalFormulaAsCont(formulaRegistry.surfaceArea.rectangle, (x) => x.b, "\u0161\xED\u0159ka", dim2.length)
      )
    )
  };
}
    ...
    c: {
      deductionTree: deduce(
        deduce(
          ...
          ctorLinearEquation("2D kino", { entity }, "x")
        ),
        triListky,
        ctor("rate")
      )
      ...
      deduce(
        deduce(
          deduce(
            deduce(
              polomer,
              evalFormulaAsCont(formulaRegistry.circumReference.circle, (x) => x.o, "pizza", dim2.length)
            ),
            cont("pizza", 8, "kousek"),
            ctor("rate")
          ),
...
function krychle2() {
  ...
  const kratsiStranKvadr = deduce(
    hranaKrychle,
    ...thirdProduct("nejkrat\u0161\xED strana kv\xE1dr")
  );
  return {
    deductionTree: deduce(
      deduce(
        deduce(
          ...
          hranaKrychle,
          hranaKrychle,
          evalFormulaAsCont(formulaRegistry.surfaceArea.cuboid, (x) => x.S, "kv\xE1dr", dim2.area)
        ),
        ctorUnit("dm2")
      ),
      ctorOption("D", 4.8)
...
function uhel() {
  const dopoctenyTroj = deduce(
    contAngle("vrcholov\xFD", 43),
    deduce(
      contAngle("zn\xE1m\xFD \xFAhel", 58),
      compAngle("zn\xE1m\xFD \xFAhel", "B", "opposite")
    ),
    triangleAngle("C")
  );
  return {
...
function kava() {
  ...
  const entityBase = { entity: "balen\xED" };
  return {
    deductionTree: deduce(
      deduce(
        rate("luxusn\xED k\xE1va", 400, entity, entityBase),
        compRelative("luxusn\xED k\xE1va", "standardn\xED k\xE1va", 1 / 4)
      ),
      cont("standardn\xED k\xE1va", 2, entityBase.entity)
    )
  };
}

// src/ctedu/2026-03-31/index.ts
var __default13 = createLazyMap({
  4.1: () => bazen3().litry,
  4.2: () => bazen3().milimetry,
  6.1: () => finacniMatika2().vraceno,
  6.2: () => finacniMatika2().urokPoDani,
  6.3: () => finacniMatika2().cena
});
function bazen3() {
  const dim2 = dimensionEntity();
  const obsahDna = deduce(
    contLength("baz\xE9n", 4, "m"),
    contLength("baz\xE9n", 2.5, "m"),
    evalFormulaAsCont(formulaRegistry.surfaceArea.rectangle, (x) => x.S, "dno", dimensionEntity("m").area)
  );
  const vyska = contLength("baz\xE9n", 2);
  ...
  const obsahDnaVLitrech = deduce(
    deduce(
      obsahDnaVCm,
      vyska,
      productCombine("baz\xE9n", dim2.volume)
    ),
    ctorUnit("l")
  );
  return {
...
function finacniMatika2() {
  const entity = "K\u010D";
  const pujckaLabel = "vyp\u016Fj\u010Den\xE1 \u010D\xE1stka";
  const vkladLabel = "vklad";
  const urokLabel = "\xFArok";
  const srazkovaDan = percent(urokLabel, "da\u0148", 15);
  const pujcka = cont(pujckaLabel, 18e3, entity);
  const vklad = cont(vkladLabel, 8e5, entity);
  const urokKeVkladu = deduce(
    vklad,
    percent(vkladLabel, urokLabel, 3)
  );
  return {
    ...
    urokPoDani: {
      deductionTree: deduce(
        deduce(
          ...
          ctorDifference("\xFArok po ode\u010Dten\xED dan\u011B")
        ),
        ctorOption("D", 20400)
      )
    },
    cena: {
      deductionTree: deduce(
        deduce(
          deduce(
            cont(["notebook", "na za\u010D\xE1tku"], 25e3, entity),
            compRelativePercent("pot\xE9", "na za\u010D\xE1tku", -15)
          ),
          compRelativePercent("nov\u011B", "pot\xE9", 10)
        ),
        ctorOption("F", "jin\xFD v\xFDsledek")
      )
    }
...
// src/ctedu/2026-04-07/index.ts
var __default14 = createLazyMap({
  1: () => stupne(),
  2: () => papir(),
  3.1: () => cesta().rozdil,
  3.2: () => cesta().doba,
  4.1: () => obdelnik4().delka,
  4.2: () => obdelnik4().milimetry,
  5: () => miska(),
  6: () => taborSporty(),
  7: () => stanekRuzi()
});
...
function cesta() {
  ...
  const max2 = contLength("Max", 200, "m");
  const igor = deduce(
    deduce(
      max2,
      ...quaterProduct("polom\u011Br")
    ),
    evalFormulaAsCont(formulaRegistry.circumReference.circle, (x) => x.o, "Igor", dim2.length)
  );
  return {
    rozdil: {
      deductionTree: deduce(
    ...
    doba: {
      deductionTree: deduce(
        deduce(
          last(igor),
          rate("Igor", 120, dim2.length, { entity: "\u010Das", unit: "min" })
        ),
        ctorUnit("s")
      )
    }
...
function obdelnik4() {
  ...
  const delka = deduce(
    stranaCtverce,
    cont("obd\xE9ln\xEDk", 6, "\u010Dtverec"),
    product("obd\xE9ln\xEDk, d\xE9lka", [], dim2.length)
  );
  const sirka = deduce(
    last(stranaCtverce),
    cont("obd\xE9ln\xEDk", 2, "\u010Dtverec"),
    product("obd\xE9ln\xEDk, \u0161\xED\u0159ka", [], dim2.length)
  );
  return {
    delka: {
      deductionTree: delka
  ...
  return {
    deductionTree: deduce(
      deduce(
        deduce(
          bila,
          ratio("celkem", "b\xEDl\xE9", 1 / 3)
        ),
        last(bila),
        ctorDifference("zbylo b\xEDl\xE9")
      ),
      ctorOption("B", 10)
    )
  };
...
function stanekRuzi() {
  ...
  const bile = deduce(
    ...
    compRelativePercent("modr\xE9", "b\xEDl\xE9", -20)
  );
  return {
    deductionTree: deduce(
      deduce(
...
var word_problems_default = createLazyMap({
  ...
  "2026-03-17": () => __default11,
  "2026-03-24": () => __default12,
  "2026-03-31": () => __default13,
  "2026-04-07": () => __default14
});
export {
