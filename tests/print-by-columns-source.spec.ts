...
for (const pageName of ["print", "solution"]) {
  for (const source of ["ctedu", "cermat"]) {
    ...
    for (const code of folders) {
      test(`${pageName} - ${source} - ${code}`, async ({ page }) => {
        await page.goto(getTestUrl(`${source}/${pageName}-${code}`));
        ...
        for (const { pageSize, columnsCount, orientation } of printRequests) {
          ...
          await page.pdf({
            ...
            path: ['generated', `${pageName == 'print' ? subject: `${pageName}-${subject}`}`, `${formatPdfFileName({ pageSize, columnsCount, orientation })}`].concat(`${code}.pdf`).join("/")
          })
        }
      });
    }
